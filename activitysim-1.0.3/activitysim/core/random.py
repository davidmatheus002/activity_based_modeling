...
class SimpleChannel(object):
    ...
    def _normal_generators_for_df(self, df, mu, sigma):
        """
        ...
        for row in df_row_states.itertuples():
            ...
            if row.offset:
                # https://numpy.org/doc/1.16/reference/generated/numpy.random.RandomState.rand.html#numpy.random.RandomState.rand
                # consume rands, uniform distribution over [0, 1)
                prng.normal(mu, sigma, row.offset)

            yield prng

    def normal_random_for_df(self, df, step_name, mu, sigma, n=1):
        """
        Return n normal distributed floating point random numbers in range for each row in df
        ...
        generators = self._normal_generators_for_df(df, mu, sigma)

        rands = np.asanyarray([prng.normal(mu, sigma, n) for prng in generators])
...
class Random(object):
    ...
    def set_base_seed(self, seed=None):
        ...
        if seed is None:
            ...
            self.base_seed = seed
            logger.info("Set random seed base to %s" % self.base_seed)
    ...
    def normal_random_for_df(self, df, mu, sigma, n=1):
        channel = self.get_channel_for_df(df)
        rands = channel.normal_random_for_df(df, self.step_name, mu, sigma, n)
        return rands
