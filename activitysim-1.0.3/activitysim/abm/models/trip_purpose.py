...
def choose_intermediate_trip_purpose(trips, probs_spec, estimator, trace_hh_id, trace_label):
    ...
    # 2021/07/31 H.Zhou: correct the error that the university student should NOT have primary purpose as "school"
    tmp = trips[(trips.primary_purpose == "school") & (trips.person_type == "PTYPE_UNIVERSITY")]
    logger.warning("{0} trips having primary purpose SCHOOL while person type is UNIVERSITY, they will be corrected".format(len(tmp)))
    trips.loc[
        (trips.primary_purpose == "school") & (trips.person_type == "PTYPE_UNIVERSITY"), "primary_purpose"] = "univ"
