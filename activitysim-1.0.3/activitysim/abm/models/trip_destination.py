...
def choose_trip_destination(
    ...
    # destination_sample = destination_sample.merge(trips['activity_type'], left_index=True, right_index=True)
    # destination_sample['free_parking_cap'] = reindex(land_use.FreeParkingCapacity, destination_sample.dest_taz)
...
def run_trip_destination(
    ...
    # trippurpose_2_activity_type_int = config.read_model_settings(model_settings.get('LOGSUM_SETTINGS'))['ACTIVITY_TYPE_MAPPING']
    # trips['activity_type'] = trips.purpose.map(trippurpose_2_activity_type_int)
    # if trips.activity_type.isna().any():
    #     logger.error("activty_type should not be NaN")
