...
def run_destination_sample(
    ...
    choices['tour_type'] = tours.tour_type
    choices['number_of_participants'] = tours.number_of_participants
    choices['tour_category'] = tours.tour_category
    return choices
...
def run_tour_destination(
    ...
    # tourtype_2_activity_type_int = config.read_model_settings(model_settings.get('LOGSUM_SETTINGS'))['ACTIVITY_TYPE_MAPPING']
    # tours['activity_type'] = tours.tour_type.map(tourtype_2_activity_type_int)
    ...
    for segment_name in segments:
        ...
        # location_sample_df = location_sample_df.merge(choosers['activity_type'], left_index=True, right_index=True)
        # location_sample_df['free_parking_cap'] = reindex(land_use.FreeParkingCapacity, location_sample_df.alt_dest)
