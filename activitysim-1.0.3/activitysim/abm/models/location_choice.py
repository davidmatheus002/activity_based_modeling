...
def _location_sample(
    ...
    # Added by David on 29-06-2022
    if 'SIMPLE_RANDOM_SAMPLING' in model_settings:
        if model_settings['SIMPLE_RANDOM_SAMPLING']:
            persons = choosers.index.values
            zones = alternatives.index.values
            sampling_size = round(len(zones) / 8)
            persons = np.repeat(persons, sampling_size)
            logger.info(f"Using SIMPLE_RANDOM_SAMPLING, sample size: {sampling_size}")
            destinations = []
            for _ in persons:
                dest_sample = np.random.choice(zones, sampling_size, replace=False)
                destinations.extend(np.sort(dest_sample))
            choices = pd.DataFrame(zip(persons, destinations), columns=['person_id', 'alt_dest'])
            choices.set_index('person_id', inplace=True)
            choices['prob'] = 1 / sampling_size
            choices['pick_count'] = 1

    # Presampling info obtained from the survey can be swapped in so that ActivitySim does not need to compute sampling
    elif 'SURVEY_SAMPLE_TABLE' in model_settings:
        logger.info(f"Using SURVEY_SAMPLE_TABLE: {model_settings['SURVEY_SAMPLE_TABLE']}")
        filepath = config.config_file_path(model_settings['SURVEY_SAMPLE_TABLE'])
        sample_table = pd.read_csv(filepath)
        # choosers_unique_home_ids = choosers.home_zone_id.unique().tolist()
        choosers_home_id = choosers['home_zone_id'].to_frame(name='home_zone_id')
        sample_unique_home_ids = sample_table.home_zone_id.unique().tolist()
        choosers_home_id = choosers_home_id.loc[choosers_home_id.home_zone_id.isin(sample_unique_home_ids)]
        probabilities_segments = []
        for chooser in choosers_home_id.index.tolist():
            home_id = choosers_home_id.loc[chooser, 'home_zone_id']
            segment = sample_table.loc[sample_table.home_zone_id == home_id].copy()
            segment['person_id'] = chooser
            segment.set_index('person_id', drop=True, inplace=True)
            probabilities_segments.append(segment)
        sample_table = pd.concat(probabilities_segments).drop('home_zone_id', axis=1)
        ...
        # choosers_home_ids = choosers.home_zone_id.unique().tolist()
        # sample_table = sample_table.loc[sample_table.home_zone_id.isin(choosers_home_ids)]
        # sample_table_home_ids = sample_table.home_zone_id.unique().tolist()
        # choosers_to_sample = choosers.copy().loc[~choosers.home_zone_id.isin(sample_table_home_ids)]
        # choosers = choosers_to_sample

        choices_person_ids = choices.index.unique().tolist()
        sample_table = sample_table.loc[sample_table.index.isin(choices_person_ids)]
        sample_table_person_ids = sample_table.index.unique().tolist()
        # sample_table.set_index('person_id', drop=True, inplace=True)
        choices_sliced = choices.loc[~choices.index.isin(sample_table_person_ids)]

        # sample_table.set_index('home_zone_id', drop=True, inplace=True)
        # sample_table.rename(columns={'alt_dest': alt_dest_col_name}, inplace=True)
        # if choosers.empty:
        # choices = sample_table
        # else:
        choices = pd.concat([choices_sliced, sample_table])
        choices = choices.rename_axis('person_id').sort_values(by=['person_id', alt_dest_col_name])

        del probabilities_segments
        del sample_table

    else:
        # if not choosers.empty: # if using survey sampling this should not be necessary, leaving just in case
        choices = interaction_sample(
            choosers,
            alternatives,
            spec=spec,
    ...
    # if 'SURVEY_SAMPLE_TABLE' in model_settings:
    #     choices_person_ids = choices.index.unique().tolist()
    #     sample_table = sample_table.loc[sample_table.index.isin(choices_person_ids)]
    #     sample_table_person_ids = sample_table.index.unique().tolist()
    #     # sample_table.set_index('person_id', drop=True, inplace=True)
...
def workplace_location(
    ...
    persons_merged['activity_type'] = 2  # work
    ...
    persons_merged.local.drop("activity_type", 1, inplace=True)  # drop this column
...
def school_location(
    ...
    # TODO: H.Zhou. assign activty type here since there is no 'tour_type' yet, to be improved.
    persons_merged['activity_type'] = 5 # onderwijs
