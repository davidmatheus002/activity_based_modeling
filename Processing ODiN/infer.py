...
"""
Script to infer values from survey data that are needed for ActivitySim estimation

Modified on Thu Jun 16 09:42 2022
...
# TODO figure out if I really need this part
...
def infer_cdap_activity(persons, tours, joint_tour_participants):
    ...
    # mandatory_tour_types = ['work', 'school', 'university']
...
def infer_mandatory_tour_frequency(persons, tours):
    ...
    num_university_tours = \
        tours[tours.tour_type == 'university'].\
        groupby('person_id').size().reindex(persons.index).fillna(0).astype(np.int8)

    # FIXME verify this is ok
...
def patch_tour_ids(persons, tours, joint_tour_participants):
    # FIXME to not consider joint tour participants
...
def infer(configs_dir, input_dir, output_dir):
    # FIXME take out joint tour participants
